...
local function checkFollowStatus(targetUsername)
    ...
    if not httpRequest then
        return false, "HTTP_UNSUPPORTED"
    end

    local success, isFollowing = pcall(function()
        local userReq = httpRequest({
            Url = "https://users.roblox.com/v1/usernames/users",
            Method = "POST",
            Headers = {["Content-Type"] = "application/json"},
            Body = HttpService:JSONEncode({usernames = {targetUsername}, excludeBannedUsers = false})
        })

        local targetId = nil
        if userReq and userReq.Success then
            local data = HttpService:JSONDecode(userReq.Body)
            if data and data.data and data.data[1] then
                targetId = data.data[1].id
            end
        end

        if not targetId then return false end

        local followReq = httpRequest({
            Url = "https://friends.roblox.com/v1/users/" .. tostring(player.UserId) .. "/followings?limit=100",
            Method = "GET"
        })

        if followReq and followReq.Success then
            local followData = HttpService:JSONDecode(followReq.Body)
            if followData and followData.data then
                for _, item in ipairs(followData.data) do
    ...
    return (success and isFollowing == true), "API_CHECK"
end
...
local loadMainGUI, showLoadingOverlay, showFollowModal
local hubGuiRef = nil

-- ================== ROBLOX FOLLOW MODAL ==================
showFollowModal = function(onSuccess)
    local folGui = Instance.new("ScreenGui", player:WaitForChild("PlayerGui"))
    folGui.Name = "PalRyzFollowModal"
    folGui.ResetOnSpawn = false
    folGui.DisplayOrder = 100
    folGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

    local overlay = Instance.new("Frame", folGui)
    overlay.Size = UDim2.new(1, 0, 1, 0)
    overlay.BackgroundColor3 = Color3.fromRGB(5, 3, 12)
    overlay.BackgroundTransparency = 0.35

    local modal = Instance.new("Frame", folGui)
    modal.Size = UDim2.new(0, 310, 0, 230)
    modal.AnchorPoint = Vector2.new(0.5, 0.5)
    modal.Position = UDim2.new(0.5, 0, 0.5, 0)
    modal.BackgroundColor3 = Theme.BG
    corner(modal, 16)
    glow(modal, Color3.fromRGB(0, 255, 204))
    gradient(modal, Color3.fromRGB(24, 16, 48), Color3.fromRGB(12, 8, 24), 135)

    local title = Instance.new("TextLabel", modal)
    title.Size = UDim2.new(1, -20, 0, 24)
    title.Position = UDim2.new(0, 10, 0, 14)
    title.BackgroundTransparency = 1
    title.Text = "REQUIREMENT ACCESS"
    title.TextColor3 = Color3.fromRGB(0, 255, 204)
    title.Font = Enum.Font.GothamBold
    title.TextSize = 13

    local desc = Instance.new("TextLabel", modal)
    desc.Size = UDim2.new(1, -30, 0, 42)
    desc.Position = UDim2.new(0, 15, 0, 42)
    desc.BackgroundTransparency = 1
    desc.Text = "Follow profil Roblox @" .. TARGET_FOLLOW_USER .. " untuk membuka Akses Penuh PalRyz HUB!"
    ...
    openBtn.Position = UDim2.new(0, 15, 0, 134)
    openBtn.BackgroundColor3 = Theme.Card
    openBtn.Text = "Open Profile"
    openBtn.TextColor3 = Theme.Text
    openBtn.Font = Enum.Font.GothamBold
    openBtn.TextSize = 10
    corner(openBtn, 10)
    stroke(openBtn, Theme.Stroke, 1)
    ripple(openBtn)

    local verifyBtn = Instance.new("TextButton", modal)
    verifyBtn.Size = UDim2.new(0.5, -20, 0, 36)
    verifyBtn.Position = UDim2.new(0.5, 5, 0, 134)
    verifyBtn.BackgroundColor3 = Theme.Accent
    verifyBtn.Text = "Cek / Verify"
    verifyBtn.TextColor3 = Color3.new(1, 1, 1)
    verifyBtn.Font = Enum.Font.GothamBold
    verifyBtn.TextSize = 10
    corner(verifyBtn, 10)
    gradient(verifyBtn, Theme.Accent, Theme.Second, 45)
    ripple(verifyBtn)

    local statusLabel = Instance.new("TextLabel", modal)
    statusLabel.Size = UDim2.new(1, -30, 0, 18)
    statusLabel.Position = UDim2.new(0, 15, 0, 182)
    statusLabel.BackgroundTransparency = 1
    statusLabel.Text = "Tekan 'Cek / Verify' setelah mem-follow"
    ...
    verifyBtn.MouseButton1Click:Connect(function()
        ...
        isChecking = true
        statusLabel.Text = "Memeriksa status follow..."
        statusLabel.TextColor3 = Color3.fromRGB(255, 200, 100)

        task.spawn(function()
            local isFollowing = checkFollowStatus(TARGET_FOLLOW_USER)
            isChecking = false

            if isFollowing then
                statusLabel.Text = "Follow Terdeteksi! Akses Diberikan ✓"
                statusLabel.TextColor3 = Color3.fromRGB(100, 255, 140)
                notify("Follow Verified 👑", "Terimakasih telah mem-follow @" .. TARGET_FOLLOW_USER, "success", 4)
                task.wait(0.6)
                tween(modal, 0.3, {Size = UDim2.new(0, 310, 0, 0), BackgroundTransparency = 1})
                task.wait(0.3)
                folGui:Destroy()
                if onSuccess then onSuccess() end
            else
                statusLabel.Text = "Belum Follow! Silakan follow @" .. TARGET_FOLLOW_USER
                statusLabel.TextColor3 = Color3.fromRGB(255, 90, 90)
                notify("Belum Follow", "Silakan follow @" .. TARGET_FOLLOW_USER .. " terlebih dahulu!", "warn", 3)
            end
        end)
    end)
end
